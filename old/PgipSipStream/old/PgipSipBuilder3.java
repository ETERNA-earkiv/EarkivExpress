package com.example.restservice.PgipSipStream.old;
...
public class PgipSipBuilder3 {
  ...
  private Job job;
  private Flux<DataBuffer> zipDataBufferFlux;

  public PgipSipBuilder3() {
    Sinks.Many<DataBuffer> sink = Sinks.many().unicast()
        .onBackpressureBuffer(Queues.<DataBuffer>small().get(),
            Disposables.never());
    Flux<DataBuffer> zipDataBufferFlux = sink.asFlux().doOnCancel(sink::tryEmitComplete);
  }

  public void createZip() throws IOException {
    pipeOut = new PipedOutputStream();
    pipedInputStream = new PipedInputStream(pipeOut, 64 * 1024);
    zipArchiveOutputStream = new ZipArchiveOutputStream(pipeOut);
  }
  ...
  public Mono<Void> saveZip() throws IOException {
    return Mono.fromRunnable(() -> {
          try (InputStream in = pipedInputStream;
              OutputStream fileOut = new BufferedOutputStream(new FileOutputStream(
                  "/Volumes/ramdisk/upload/" + UUID.randomUUID() + ".zip"))) {
            ...
            throw new RuntimeException("Error writing test ZIP file", e);
          }
        })
        .subscribeOn(Schedulers.boundedElastic())  // run blocking file write on elastic thread
        .then()
        .doFinally(sig -> {
          try {
